...
function sound(kind,ev){if(muted||!audio)return;try{const t=audio.currentTime,pitch=ev&&ev.pitch||'fast',fire=pitch==='fire',slow=pitch==='slow',slider=pitch==='slider';
 ...
 if(kind==='windup'){const base=fire?440:slow?170:slider?260:300,end=fire?760:slow?260:slider?400:500;tone('sine',base,.24,fire?.038:slow?.02:.028,end);if(fire)tone('sawtooth',base*.65,.2,.014,end*.75);else if(!slow)tone('triangle',base*.85,.18,.012,end*.7)}
 else if(kind==='pitch'){const base=fire?400:slow?150:slider?250:230;tone(fire?'square':'triangle',base,.09,fire?.055:slow?.026:.04,base*.35);if(fire)tone('sine',base*1.6,.07,.022,base*.55);else if(slider)tone('sine',base*1.2,.08,.016,base*.5)}
...
function draw(){
 ...
 drawHoldLockAim();drawFtueGuide();ctx.restore();if(flash>0){ctx.fillStyle='#ff765329';ctx.fillRect(0,0,W,H)}drawNearMissVignette();
}

'use strict';
/**
 * Night regression smoke — 2026-09-14 post-polish (locked toast + daily note + multi-title unlock)
...
(async () => {
  ...
  try {
    ...
    step('title unlock note lists first_perfect too', /첫 퍼펙트/.test(unlock.text), unlock.text);
    step('daily clear note has 칭호 해금 on first unlock', /칭호 해금/.test(resultNote), resultNote);
    await shot(page, '06-result-daily-clear.png');

    // Locked toast inside catalog (post 670011f)
    await page.evaluate(() => {
      const btn = document.querySelector('#titles-open') || document.querySelector('#title-chip');
      if (btn) btn.click();
    });
    await page.waitForTimeout(200);
    const lockedToast = await page.evaluate(() => {
      ...
      if (!el) return { ok: false, reason: 'no toast el' };
    ...
    step('locked toast visible in catalog', lockedToast.ok && lockedToast.h > 0, JSON.stringify(lockedToast));
    await page.evaluate(() => { const d = document.querySelector('dialog.titles-dialog'); if (d && d.open) d.close(); });
    await page.waitForTimeout(100);

    // Continue same daily: result note must NOT falsely append 「· 칭호 해금」
    await page.evaluate(() => { if (typeof start === 'function') start('continue'); });
    await page.waitForTimeout(400);
    await page.evaluate(() => {
      game.stage = 2;
      game.perfects = 1;
    ...
    await page.waitForTimeout(400);
    const contNote = await page.evaluate(() => {
      const n = document.querySelector('.daily-result-note');
      return n ? n.textContent.trim() : '';
    });
    step('continue clear note without false 칭호 해금', contNote === '오늘의 승부 클리어!', contNote);
    await shot(page, '06b-continue-clear-no-false-unlock.png');

...
class BaseballGame {
 ...
 const pattern=n<=4?['fast','fast','slow','fire'][n-1]:pool[Math.floor(this.random()*pool.length)];
 const type={double:'fast',changeFast:'slow',fireTwin:'fire',twin:'fast'}[pattern]||pattern;
 this.windup={pattern,type,t:this.fury?.65:.8,tx:Math.max(85,Math.min(395,this.player.x+(this.random()-.5)*40))};
 this.emit('windup',{pitch:type,pattern});
 }
 throwBall(){
 const w=this.windup,scale=Math.max(.68,1-(this.stage-1)*.08),duration=type=>(type==='slow'?2:type==='slider'?1.65:1.32)*scale;
 ...
 getPerfectZone(){const scale=this.upgradeValue('vision','scale',1);return {x:32*scale,y:17*scale}}
 getUpgradeChoices(){
...
swing(){if(this.state!=='playing'||this.cooldown>0)return false;this.cooldown=this.getSwingCooldown();this.swingAnim=.21;const z=this.zone;const reach=this.getWideReach();const candidates=this.balls.filter(b=>!b.resolved&&b.t>=0&&b.type!=='fire'&&((b.x-z.x)/reach)**2+((b.y-z.y)/48)**2<=1).sort((a,b)=>Math.abs(a.y-z.y)-Math.abs(b.y-z.y));const ball=candidates[0];if(ball){ball.resolved=true;const perfectZone=this.getPerfectZone();const perfect=((ball.x-z.x)/perfectZone.x)**2+((ball.y-z.y)/perfectZone.y)**2<=1;this.hits++;this.combo++;this.bestCombo=Math.max(this.bestCombo,this.combo);if(perfect)this.perfects++;let rally=false;let damage=perfect?(this.meta.slugger?this.getPerfectDamage():20):10;if(this.combo>=2&&!this.rallyReady){this.rallyCount++;this.rallyReady=true;rally=true;damage+=this.upgradeValue('homer','bonus',25);this.emit('rally',{count:this.combo,damage});this.combo=0;}this.returns.push({x:ball.x,y:ball.y,sx:ball.x,sy:ball.y,t:0,damage,perfect,rally});this.emit(perfect?'perfect':'hit',{x:ball.x,y:ball.y,combo:this.combo})}else{const guarded=this.combo>0&&this.comboGuards>0;if(guarded)this.comboGuards--;else this.combo=0;this.misses++;this.emit('whiff');this.emit('reaction',{line:guarded?'콤보 보호! · 남은 '+this.comboGuards+'회':'벌써 휘둘렀어?'})}return true}
 update(dt,dx=0,dy=0){if(this.state!=='playing')return;this.time+=dt;this.cooldown=Math.max(0,this.cooldown-dt);this.swingAnim=Math.max(0,this.swingAnim-dt);this.inv=Math.max(0,this.inv-dt);this.move(dx,dy,dt);if(this.windup){this.windup.t-=dt;if(this.windup.t<=0)this.throwBall()}else{this.nextPitch-=dt;if(this.nextPitch<=0)this.prepare()}
 for(const b of this.balls){if(b.resolved)continue;b.t+=dt;if(b.t<0)continue;if(b.announced===false){b.announced=true;this.emit(b.twin?'twin':'pitch',{pitch:b.type})}const q=b.t/b.duration,sx=b.sx??240,sy=b.sy??290;b.x=sx+(b.tx-sx)*q+(b.curve||0)*Math.sin(Math.PI*Math.min(1,q));b.y=sy+(650-sy)*q;if(Math.hypot(b.x-this.player.x,b.y-this.player.y)<24){b.resolved=true;this.combo=0;this.emit('reaction',{line:'이번 공은 내 거야.'});if(this.inv<=0){this.inv=1.1;if(this.hp===1&&this.lastStands>0){this.lastStands--;this.emit('lastStand')}else{this.hp--;this.emit('damage',{x:this.player.x,y:this.player.y})}if(this.hp<=0){this.state='lost';this.emit('end');break}}}else if(b.y>785){b.resolved=true;if(b.type==='fire')this.dodges++;else this.combo=0}}
 this.balls=this.balls.filter(b=>!b.resolved);if(this.state!=='playing')return;for(const r of this.returns){r.t+=dt;const q=Math.min(1,r.t/.38);r.x=r.sx+(240-r.sx)*q;r.y=r.sy+(290-r.sy)*q;if(q>=1){this.boss=Math.max(0,this.boss-r.damage);this.emit('impact',{damage:r.damage,perfect:r.perfect});if(r.rally)this.rallyReady=false;r.done=true;if(this.boss<=0){this.state='won';this.emit('end');break}}}this.returns=this.returns.filter(r=>!r.done);if(this.boss<=this.bossMax*.5&&!this.fury&&this.state==='playing'){this.fury=true;this.emit('fury');this.emit('reaction',{line:'이제 진짜 던진다!'})}}

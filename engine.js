...
class BaseballGame {
 ...
 const early=n<=4;let pattern,type,aimSpan=early?120:40;
 if(early){pattern=['fast','slow','fast','fire'][n-1]}
  ...
  if(trackerOk){pattern='fast';aimSpan=20;this.lastTrackerAt=this.time}
  else pattern=this.pickSoftHeatPattern(pool);
 }
 type={double:'fast',changeFast:'slow',fireTwin:'fire',twin:'fast'}[pattern]||pattern;
 ...
 pickSoftHeatPattern(pool){
  const {fireW,fireTwinW}=this.softHeatBands();
  const weights=pool.map(p=>p==='fire'?fireW:p==='fireTwin'?fireTwinW:1);
 ...
 this.windup=null;this.nextPitch=finish+(this.pitchCount>=5?this.softHeatGap():(this.fury?.2:.5));this.emit('pitch',{pitch:w.type});
 }
